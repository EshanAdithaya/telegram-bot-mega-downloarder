...
Ready to receive your MEGA link! 📁
    ...
    async def process_mega_folder(self, mega_url: str, update: Update, status_msg):
        ...
        try:
            ...
            folder_contents = m.get_files_in_folder(mega_url)
            
            if not folder_contents:
                await status_msg.edit_text("❌ No files found in the folder or invalid link.")
                return
            
            # Filter media files
            media_files = self.filter_media_files(folder_contents)
            ...
            for i, (file_id, file_info) in enumerate(media_files.items(), 1):
                try:
                    ...
                    m.download_file(file_id, dest_path=self.temp_dir, dest_filename=filename)
